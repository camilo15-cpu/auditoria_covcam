...
![XSS Reflejado](img_reycam/xss_reycam.png)
*Payload utilizado:* `<script>alert('XSS')</script>`

### 2. Análisis Técnico
La aplicación web refleja de manera directa los parámetros suministrados por el usuario en las respuestas HTML sin realizar codificación de salida (Output Encoding). Esto causa que el navegador de la víctima confunda el texto de entrada con scripts del lado del cliente legítimos y los ejecute automáticamente en su contexto de navegación.
