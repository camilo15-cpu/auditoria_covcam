...
![Inyección SQL](img_reycam/sqli_reycam.png)
*Payload utilizado:* `' OR '1'='1`

### 2. Análisis Técnico
La vulnerabilidad se presenta en el formulario de consulta de datos del portal web de la municipalidad. El sistema toma la entrada del usuario directamente y la concatena en la consulta SQL sin sanitizar. El payload altera la estructura lógica transformando la condición en una sentencia siempre verdadera (`WHERE id = '' OR '1'='1'`), lo que fuerza al motor de base de datos a retornar la totalidad de los registros de la tabla afectada.
...
*   **Prevención:** Implementar consultas parametrizadas (Prepared Statements) con asignación estricta de tipos de datos, impidiendo que los valores de entrada sean interpretados por el intérprete SQL como comandos ejecutable.
